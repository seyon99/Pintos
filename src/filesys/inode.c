...
struct inode_disk
  {
    ...
    block_sector_t direct[12];          /* 6 KB from direct pointers */
    block_sector_t indirect;            /* 64 KB from level 1 indirect pointer */
    block_sector_t dbl_indirect;        /* 8 MB from level 2 indirect pointer */
...
inode_get_sector (const block_sector_t inode_sector, const off_t pos)
{
  ...
  {
    cache_read(fs_device, inode_sector, &sector, offsetof(struct inode_disk, direct) + idx * sizeof(block_sector_t), sizeof(block_sector_t));
  }
  else if (idx < DIRECT_BLOCKS + INDIRECT_BLOCKS)
  {
    cache_read(fs_device, inode_sector, &sector, offsetof(struct inode_disk, indirect), sizeof(block_sector_t));
    cache_read(fs_device, sector, &sector, (idx - DIRECT_BLOCKS) * sizeof(block_sector_t), sizeof(block_sector_t));
  }
  else if (idx < DIRECT_BLOCKS + INDIRECT_BLOCKS + DBL_INDIRECT_BLOCKS)
  {
    cache_read(fs_device, inode_sector, &sector,offsetof(struct inode_disk, indirect), sizeof(block_sector_t));

    int dbl_num = (idx - DIRECT_BLOCKS - INDIRECT_BLOCKS) / INDIRECT_BLOCKS;
    cache_read(fs_device, sector, &sector, dbl_num * sizeof(block_sector_t), sizeof(block_sector_t));

    int dbl_offset = (idx - DIRECT_BLOCKS - INDIRECT_BLOCKS) % INDIRECT_BLOCKS;
    cache_read(fs_device, sector, &sector, dbl_offset * sizeof(block_sector_t), sizeof(block_sector_t));
  }
...
inode_create_sector (const block_sector_t inode_sector, const off_t pos)
{
  ...
  if (idx < DIRECT_BLOCKS)
  {
    ...
    if (!sector)
    {
      if (free_map_calloc(&sector))
        cache_write(fs_device, inode_sector, &sector, offsetof(struct inode_disk, direct) + idx * sizeof(block_sector_t), sizeof(block_sector_t));
      else
        return 0;
    }
    return sector;
  ...
  {
    cache_read(fs_device, inode_sector, &indirect, offsetof(struct inode_disk, indirect), sizeof(block_sector_t));

    if (!indirect)
    {
      if (free_map_calloc(&indirect))
        cache_write(fs_device, inode_sector, &indirect, offsetof(struct inode_disk, indirect), sizeof(block_sector_t));
      else
        return 0;
    }

    idx -= DIRECT_BLOCKS;
    cache_read(fs_device, indirect, &sector, idx * sizeof(block_sector_t), sizeof(block_sector_t));

    if (!sector)
    {
      if (free_map_calloc(&sector))
        cache_write(fs_device, indirect, &sector, idx * sizeof(block_sector_t), sizeof(block_sector_t));
      else
        return 0;
    }
    return sector;
  ...
  {
    cache_read(fs_device, inode_sector, &dbl_indirect, offsetof(struct inode_disk, dbl_indirect), sizeof(block_sector_t));

    if (!dbl_indirect)
    {
      if (free_map_calloc(&dbl_indirect))
        cache_write(fs_device, inode_sector, &dbl_indirect, offsetof(struct inode_disk, dbl_indirect), sizeof(block_sector_t));
      else
        return 0;
    }

    idx -= DIRECT_BLOCKS + INDIRECT_BLOCKS;
    cache_read(fs_device, dbl_indirect, &indirect, (idx / INDIRECT_BLOCKS) * sizeof(block_sector_t), sizeof(block_sector_t));

    if (!indirect)
    {
      if (free_map_calloc(&indirect))
        cache_write(fs_device, dbl_indirect, &indirect, (idx / INDIRECT_BLOCKS) * sizeof(block_sector_t), sizeof(block_sector_t));
      else
        return 0;
    }

    cache_read(fs_device, indirect, &sector, (idx % INDIRECT_BLOCKS) * sizeof(block_sector_t), sizeof(block_sector_t));

    if (!sector)
    {
      if (free_map_calloc(&sector))
        cache_write(fs_device, indirect, &sector, (idx % INDIRECT_BLOCKS) * sizeof(block_sector_t), sizeof(block_sector_t));
      else
        return 0;
    }
    return sector;
  ...
  if (indirect)
    {
      for (i = 0; i < INDIRECT_BLOCKS; ++i)
        {
          cache_read (fs_device, indirect, &sector, i * sizeof (block_sector_t), sizeof (block_sector_t));

          if (sector)
            free_map_release (sector);

          sector = NULL;
          cache_write (fs_device, indirect, &sector, i * sizeof (block_sector_t), sizeof (block_sector_t));
        }
  ...
  if (dbl_indirect)
    {
      for (j = 0; j < INDIRECT_BLOCKS; ++j)
        {
          cache_read (fs_device, dbl_indirect, &indirect, i * sizeof (block_sector_t), sizeof (block_sector_t));
          if (indirect)
            {
              for (i = 0; i < INDIRECT_BLOCKS; ++i)
                {
...
inode_create (block_sector_t sector, off_t length)
{
  ...
  cache_write (fs_device, sector, &length, offsetof (struct inode_disk, length), sizeof (off_t));
  cache_write (fs_device, sector, &inode_magic, offsetof (struct inode_disk, magic), sizeof (unsigned));
